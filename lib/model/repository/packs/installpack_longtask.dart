...
/// [LongTask] that installs a pack. If the pack is already installed, it does nothing.
class InstallPackLongTask extends LongTask {
  Pack pack;

  InstallPackLongTask({required Repository repository, required this.pack}) : super() {
    this.id = generateId(this.pack);
  }

  /// Generate an unique id used to identify this task.
  static String generateId(Pack pack) => "pack_install(${pack.name})";

  @override
  Future<void> doTask() async {
    if (!this.pack.isInstalled) {
